...
float bilinear_interpolate(image im, float x, float y, int c)
{
    ...
    float q1 = (buttom - y) * get_pixel(im,l,top,c) + (y - top) * get_pixel(im,l,buttom,c);

    float q2 = (buttom - y) * get_pixel(im,r,top,c) + (y - top) * get_pixel(im,r,buttom,c);

    return (r - x) * q1 + (x - l) * q2;
}
